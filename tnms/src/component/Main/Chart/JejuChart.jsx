...
export default function JejuChart() {
  ...
  useEffect(() => {
    fetch("/ChartJSON/ChartJeju.json") // 내가 만든 JSON 파일 호출용??
      .then((res) => res.json())
 ...
 const legendColors = [ // 이건 색상표 색
  ...
  const rainValue = data.RN_DAY.value === -9 ? 30 : data.RN_DAY.value; // 호우 RN_DAY 값이 -9면 30으로 임의 지정, 아니면 API 값 사용한다는 뜻
  const snowValue = data.SD_MAX.value === -9 ? 7 : data.SD_MAX.value; // 폭설
  const tempValue = data.TA_AVG.value === -9 ? -4 : data.TA_AVG.value; // 한파
  const dustValue = 27;                    // 황사
  const fireValue = "";                    // 산불
  const landslideValue = 3;               // 산사태
